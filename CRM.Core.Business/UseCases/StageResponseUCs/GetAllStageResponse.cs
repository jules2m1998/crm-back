...
public static class GetAllStageResponse
{
    public record Query(): IRequest<IEnumerable<StageResponseModel.Out>>;
    public class Handler : IRequestHandler<Query, IEnumerable<StageResponseModel.Out>>
    {
        ...
        {
            IEnumerable<StageResponse> responses = await repo.GetAllAsync(cancellationToken);
