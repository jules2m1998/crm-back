...
public class BaseRepository<TEntity> : IAsyncRepository<TEntity> where TEntity : class
{
    ...
    public virtual async Task<IReadOnlyList<TEntity>> GetAllAsync() => await Set.ToListAsync();
    ...
    public virtual async Task<IReadOnlyList<TEntity>> FindManyWhere(Expression<Func<TEntity, bool>> predicate) =>
        await Set.Where(predicate).ToListAsync();

    public virtual async Task<TEntity?> FindOneWhere(Expression<Func<TEntity, bool>> predicate) => 
